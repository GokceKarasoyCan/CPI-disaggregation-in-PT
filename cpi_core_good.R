...
df_cpi_percent <- 
  df_cpi_percent %>%
  mutate(cg_L1 = lag(core_gds_qoq, 1),
...
df_cpi_tbl <-
  ...
  mutate(D_2008Q4 = ifelse(date >= as.Date("2008-12-30") & date <= as.Date("2008-12-30"), 1, 0)) %>%
...
full_data <-
  df_cpi_tbl %>%
  dplyr::select(date,core_gds_qoq, cg_L1,cg_L2,cg_L3,cg_L4,
...
'cg_adl <- step(lm(core_gds_qoq ~ 0+ cg_L1 + cg_L2 + cg_L3 + cg_L4+
...
cg_adl <- lm(core_gds_qoq ~ 0+ cg_L1 +  eer_qoq+
                pmdef_L2+ D_2009Q2 +D_2023Q3, 
               data=full_sample)#' # ,eer_qoq +  pmdef_qoq +

summary (cg_adl)

# Coefficient test
lmtest::coeftest(cg_adl, vcov = NeweyWest(cg_adl, prewhite = FALSE))
...
t_resid_cg <- rstandard(cg_adl)
# Flag outliers (e.g., |residual| > 2.5)
outliers_cg <- which(abs(t_resid_cg) > 3)
full_sample[outliers_cg, "core_gds_qoq"]
full_sample[outliers_cg, "date"]

# Approach_2
cooks_cg <- cooks.distance(cg_adl)
# Flag influential observations (rule of thumb: > 4/n)
threshold_cg <- 4 / nrow(full_sample)
which(cooks_cg > threshold_cg)

# Approach_3
outlierTest(cg_adl)  # Bonferroni p-values for outliers
...
#plot 1
resid_cg_adl <- resid(cg_adl)
lag.plot(resid_cg_adl, diag.col = "forest green"
         , main = "Lag Scatter Plot - Services OLS_ADL MODEL")

#plot 2

res <- as.numeric(resid_cg_adl)
...
build_x <- function(t, y_dyn, data, coef_names) {
  ...
  if ("cg_L1" %in% coef_names)
    x["cg_L1"] <- ifelse(t - 1 >= 1, y_dyn[t - 1], NA)
  
  #if ("s_L3" %in% coef_names)
    #x["s_L3"] <- ifelse(t - 3 >= 1, y_dyn[t - 3], NA)
  
  # Exogenous (always actual)
  exog_vars <- setdiff(coef_names, c("(Intercept)", "cg_L1"))
...
# Show first rows
print(head(fcst_cg_ols25, 20))


#accuracy metrics
ac_cgf25 <- fcst_cg_ols25 %>%
...
print(ac_cgf25, n = Inf)

#===============================================
######## BENCHMARK AR(1) MODEL  1991-2025 #####
#===============================================
  
cg_ar25 <- lm(core_gds_qoq ~ 0 + cg_L1 ,
             data=full_sample)#'

summary (cg_ar25)

# Coefficient test
lmtest::coeftest(cg_ar25 , vcov = NeweyWest(cg_ar25, prewhite = FALSE))
...
build_x <- function(t, y_dyn, data, coef_names_ar25) {
  ...
  # Exogenous (always actual)
  exog_vars <- setdiff(coef_names_ar25, c("(Intercept)", "cg_L1"))
  for (v in exog_vars) x[v] <- data[[v]][t]
...
# Show first rows
print(head(fcst_cg_ar25, 20))

#accuracy metrics
ac_cg_ar25 <- fcst_cg_ar25 %>%
  filter(!is.na(y_actual)) %>%
  group_by(horizon) %>%
  summarise(
    n       = n(),
...
summary (cg_adl19)
# Coefficient test
lmtest::coeftest(cg_adl19, vcov = NeweyWest(cg_adl19, prewhite = FALSE))

#----------------------------------------------------------
# 3. OUT SAMPLE MULTI-STEP FORECAST WITH MODEL COEFF-FIXED
#----------------------------------------------------------
# keeping fixed the coefficients 
beta19       <- coef(cg_adl19)
...
build_x <- function(t, y_dyn, data, coef_names19) {
  ...
  # Exogenous (always actual)
  exog_vars <- setdiff(coef_names19, c("(Intercept)", "cg_L1"))
  for (v in exog_vars) x[v] <- data[[v]][t]
...
# Show first rows
print(head(fcst_cg_ols19, 20))

#accuracy metrics
ac_cgf19 <- fcst_cg_ols19 %>%
  filter(!is.na(y_actual)) %>%
  group_by(horizon) %>%
  summarise(
    n       = n(),
...
cg_ar19 <- lm(core_gds_qoq ~ 0 + cg_L1 ,
              data=train)#'

summary (cg_ar19)

# Coefficient test
lmtest::coeftest(cg_ar19 , vcov = NeweyWest(cg_ar19, prewhite = FALSE))

#----------------------------------------------------------
# 3. FORECAST UTILITIES
#----------------------------------------------------------
# keeping fixed the coefficients 
beta_ar19       <- coef(cg_ar19)
...
build_x <- function(t, y_dyn, data, coef_names_ar19) {
  ...
  # Exogenous (always actual)
  exog_vars <- setdiff(coef_names_ar19, c("(Intercept)", "cg_L1"))
  for (v in exog_vars) x[v] <- data[[v]][t]
...
# Show first rows
print(head(fcst_cg_ar19, 20))


#accuracy metrics
ac_cg_ar19 <- fcst_cg_ar19 %>%
  filter(!is.na(y_actual)) %>%
  group_by(horizon) %>%
  summarise(
    n       = n(),
...
build_x_vec <- function(t_idx, y_dyn, data, coef_namesd19) {
  ...
  if ("cg_L1" %in% coef_namesd19) x["cg_L1"] <- if (t_idx - 1 >= 1) y_dyn[t_idx - 1] else NA_real_
  #if ("s_L3" %in% coef_names19) x["s_L3"] <- if (t_idx - 3 >= 1) y_dyn[t_idx - 3] else NA_real_
  
  # exogenous present in both model and data
  exog_in_model <- setdiff(coef_namesd19, c("(Intercept)", "cg_L1"))#, "s_L3", "s_L2", "s_L4"))
...
fcst_cgr19 <- dplyr::bind_rows(fcst_eval_list) %>%
  mutate(
