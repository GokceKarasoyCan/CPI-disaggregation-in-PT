...
build_outturn_file <- function(vintage_dates, realised_data, outturn_col, label) {
  ...
  tidyr::crossing(vintage_dates, outturn_base) %>%
    mutate(
...
series_config <- list(
  list(stub = "cg", forecast_col = "core_gds_qoq_fc", outturn_col = "core_gds_qoq", label = "Core goods inflation"),
  list(stub = "services", forecast_col = "services_qoq_fc", outturn_col = "services_qoq", label = "Services inflation"),
  list(stub = "food", forecast_col = "food_at_qoq_fc", outturn_col = "food_at_qoq", label = "Food inflation")
)
...
for (cfg in series_config) {
  ...
  if (has_arrow) {
    ...
    arrow::write_parquet(forecast_df, forecast_parquet)
    arrow::write_parquet(outturn_df, outturn_parquet)
    cat(sprintf(" - %s\n", forecast_parquet))
    cat(sprintf(" - %s\n", outturn_parquet))
  }
}

cat("Finished 04_generateforecastandoutturns.R\n")
